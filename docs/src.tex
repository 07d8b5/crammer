...
% % EVIDENCE="See the parse_seconds_value() excerpt below showing errno/endptr validation and the 1..86400 bounds."
% % CONF=2
% %@END
...
Second, it enforces a semantic range: values must be within \off{1..86400}. This makes a group timer always positive and prevents absurdly long timers from silently becoming accepted “configuration”. When the check fails, the error is anchored to the current line number via \sym{set\_error\_line} and the caller gets a specific message: \off{"invalid seconds value"}.
...
% % STATE=UNVERIFIED
% % NEEDS_RESEARCH=YES
% % GAPS="Need excerpt showing the bounded loop, EINTR handling, and the final len!=0 failure."
% % NEXT_ACTION="Extract a 10–40 line excerpt from src/term.c covering write_all()."
% % SEARCH_TERMS="static int write_all MAX_WRITE_LOOPS errno == EINTR len != 0"
% % EVIDENCE="TODO: EX excerpt from src/term.c write_all"
% % CONF=0
% %@END

\subsection*{Code excerpt: bounded \sym{write\_all} (TODO)}
\begin{lstlisting}
/* TODO: EXCERPT (10–40 lines) from src/term.c write_all():
 * show MAX_WRITE_LOOPS loop, errno==EINTR retry, and len!=0 -> failure.
 * SEARCH_TERMS: static int write_all MAX_WRITE_LOOPS errno == EINTR
 */
\end{lstlisting}
...
% % STATE=UNVERIFIED
% % NEEDS_RESEARCH=YES
% % GAPS="Need excerpt showing (ready<0 && errno==EINTR) return 0, and (n<0 && errno==EINTR) return 0."
% % NEXT_ACTION="Extract a 10–40 line excerpt from src/term.c term_read_key_timeout() covering the select() error branch and the read() EINTR branch."
% % SEARCH_TERMS="ready < 0 errno == EINTR return 0 n < 0 && errno == EINTR"
% % EVIDENCE="TODO: EX excerpt from src/term.c term_read_key_timeout EINTR handling"
% % CONF=0
% %@END

\subsection*{Code excerpt: EINTR semantics (TODO)}
\begin{lstlisting}
/* TODO: EXCERPT (10–40 lines) from src/term.c term_read_key_timeout():
 * show select() EINTR -> return 0 and read() EINTR -> return 0.
 * SEARCH_TERMS: if (errno == EINTR) return 0
 */
\end{lstlisting}
...
% % STATE=UNVERIFIED
% % NEEDS_RESEARCH=YES
% % GAPS="Need excerpt showing struct app definition and the static g_app declaration in src/app.c."
% % NEXT_ACTION="Extract a 10–40 line excerpt from src/app.c covering struct app { ... } and static struct app g_app."
% % SEARCH_TERMS="struct app { struct Session static struct app g_app group_order item_order"
% % EVIDENCE="TODO: EX excerpt from src/app.c struct app / g_app"
% % CONF=0
% %@END

\subsection*{Code excerpt: app global layout (TODO)}
\begin{lstlisting}
/* TODO: EXCERPT (10–40 lines) from src/app.c showing struct app and g_app:
 * show Session/Rng members and order arrays sized by MAX_* constants.
 * SEARCH_TERMS: struct app static struct app g_app group_order item_order
 */
\end{lstlisting}
