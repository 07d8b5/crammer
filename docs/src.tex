...
{src/log.c}
{\sym{log\_simple}, \sym{log\_key}, \sym{log\_prompt}, \sym{log\_group}, \sym{log\_shuffle}, \sym{log\_input}}
...
{\sym{log\_open}, \sym{log\_close}, \sym{log\_key}, \sym{log\_prompt}, \sym{log\_group}, \sym{log\_shuffle}, \sym{log\_input}}
{Tags emitted by \filep{src/log.c}: \off{start}, \off{exit}, \off{key}, \off{prompt}, \off{file}. Tags supplied by the runner: \off{group}, \off{expired}, \off{shuffle}, \off{items}, \off{error}.}
\end{evidenceblock}
...
\texttt{file} & \sym{log\_input} (called by app) & Input identity; payload includes \texttt{cksum=<u32> len=<bytes>} and may include \texttt{path=<string>}.\\
...
\texttt{prompt} & \sym{log\_prompt} (called by runner) & Prompt drawn; payload includes \texttt{group=<g> item=<i>} plus \texttt{gck/glen} and \texttt{ick/ilen} checksums/lengths.\\
...
The \texttt{cksum} values are compatible with the ubiquitous POSIX \texttt{cksum(1)} tool: you can reproduce the file-level \texttt{cksum/len} by running \texttt{cksum <session\_file>}.
